var CalGen = (function() {
    ...
    CalGen.prototype.print = function() {
        ...
        var html = '<body><style>';
        html += '@page { size: A3; margin: 0mm; }';
        html += 'body, .page { margin: 0mm; }';
        html += '.page { float: none; break-after: always; }';
        html += '</style><title>' + this.toString() + '</title>';
        var iframe = $iframe[0];
        var step = 100 / 15 + 0.1;
        ...
        function loadResources(renderFx, callback) {
            ...
            for (var i = 0; i < this.resources.length; i++) {
                if (!this.resources[i].loaded) {
                    this.resources[i].load($.getClassName(this.resources[i]) != 'Font' ? this.success : this.loaderOpt);
                    return false;
                }
            }
