var CalGen = (function() {
	...
	CalGen.prototype.saveAs = function(format, callback) {
		...
		switch(format) {
			case 'cgp':
				LZUTF8.compressAsync(JSON.stringify(this.config), { useWebWorker: false }, function(result, error) {
					if (error !== undefined) {
					...
					for(var i = 0; i < result.length; i++)
						out += String.fromCharCode(result[i] ^ 0x89);
					...
					var file = new File([ out ], self.toString() + '.cgp', { type: 'text/plain; charset=us-ascii' });
					saveAs(file);
				});
				break;
