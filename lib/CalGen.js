var CalGen = (function() {
	var locale = {
		eng: {
			...
			holiday: [
				...
				[ [ 6, "Saint Nicholas Day", false ], [ 22, "Winter", false ], [ 24, "Christmas Eve", false ], [ 25, "Christmas Day", true ], [ 26, "Second Christmas Day", true ], [ 31, "New Year Eve", false ] ]
			],
	...
	var styleName = {
		...
		'one square': [ '0' ],
		'two squares': [ '0', '1' ],
		'three squares': [ '0', '1', '2', '3', '4' ],
		'four squares': [ '0', '1', '2', '3' ],
		'five squares': [ '0', '1', '2', '3' ],
		'six squares': [ '0', '1', '2' ]
	};
	var menuTrigger = [];
					...
					events: {
						show: function(opt) {
							...
							opt.inputs.variant.$input.on('build', build.bind(opt.inputs.variant.$input))
								.trigger('build', [ Object.values(styleName)[self.getCurrentPage().config.artwork.style] ]);

							$('#artwork-background-picker').spectrum($.extend({}, {
								showAlpha: true,
								color: self.getCurrentPage().config.artwork.bkgColor || 'rgba(0, 0, 0, 0)',
								change: function(color) {
									self.getCurrentPage().config.artwork.bkgColor = color.toRgbString();
								}
							}, defaultSpectrum));

							$('#border-color-picker').spectrum($.extend({}, {
								showAlpha: true,
								color: self.getCurrentPage().config.artwork.border.color || 'rgba(0, 0, 0, 0)',
								change: function(color) {
									self.getCurrentPage().config.artwork.border.color = color.toRgbString();
								}
							}, defaultSpectrum));
							...
							$("#border-width-slider").slider({
								...
								change: function(event, ui) { self.getCurrentPage().config.artwork.border.width = ui.value }
							});
						}
					},
					items: {
							...
							items: {
								...
								background: {
									...
									html: [
										...
										'<input id="artwork-background-picker" type="text"></input>'
									].join('')
								},
								sep2: '----',
								borderColor: {
									...
									html: [
										...
										'<input id="border-color-picker" type="text"></input>'
									].join('')
								},
								borderWidth: {
									type: 'html',
									html: [
										'<div id="border-width-slider" class="mt-2">',
						...
						remove: {
							...
							callback: function() {
								...
								if (page.config.artwork.images[index].src != '') {
									page.config.artwork.images[index] = new Image();
									self.render();
					...
					items: {
						title: {
							...
							html: [
								...
								'<input id="title-color-picker" type="text"></input>'
							].join('')
						},
						shadow: {
							type: 'html',
							html: [
								...
								'<input id="shadow-color-picker" type="text"></input>'
							].join('')
						},
						sep: '----',
						match: {
					...
					events: {
						show: function() {
							$('#subtitle-color-picker').change(function() {
								SVG.select('.cover-subtitle').members[0].fill(
					...
					items: {
						title: {
							...
							html: [
								...
								'<input id="subtitle-color-picker" type="text"></input>'
							].join('')
						},
						sep: '----',
						match: {
					...
					items: {
						...
						background: {
							...
							html: [
								...
								'<input id="code-bkg-picker" type="text"></input>'
							].join('')
						},
						sep: '----',
						restore: {
					...
					events: {
						show: function() {
							$('#outline-color-picker').spectrum($.extend({}, {
								...
								change: function(color) {
									SVG.select('.rear-outline').members[0].fill(
							...
							$('#separating-color-picker').spectrum($.extend({}, {
								...
								change: function(color) {
									SVG.select('.rear-separator').members[0].fill(
					...
					items: {
						outline: {
							...
							html: [
								...
								'<input id="outline-color-picker" type="text"></input>'
							].join('')
						},
						seperator: {
							type: 'html',
							html: [
								...
								'<input id="separating-color-picker" type="text"></input>'
							].join('')
						},
						sep: '----',
						restore: {
							...
							callback: function() {
								...
								SVG.select('.rear-outline').members[0].fill(globalConfig.rear.config.outlineColor);
								...
								SVG.select('.rear-separator').members[0].fill(globalConfig.rear.config.separatingColor);
			...
			title: {
				...
				options: {
					events: {
						show: function() {
							$('#title-color-picker').change(function() {
								SVG.select('.rear-title').members[0].fill(
									globalConfig.rear.config.titleColor = $(this).spectrum('get').toHexString()
								);
							});
							$('#shadow-color-picker').change(function() {
								SVG.select('.rear-title').members[0].stroke(
			...
			subtitle: {
				...
				options: {
					events: {
						show: function() {
							$('#subtitle-color-picker').change(function() {
								SVG.select('.rear-subtitle').members[0].fill(
	...
	var projectURL = 'https://tivian.github.io/CalGen/';
	var homeURL = 'https://tivian.github.io/';
	var lastSave = null;
	...
	function CalGen(options) {
		...
		this.reset();
		this.init();
		this[localStorage.getItem('save') != null ? 'load' : 'new'](true);
	}
	...
	CalGen.prototype.init = function() {
		...
		var resizeFx = function() {
			var rv = root.viewbox();
			draw.zoom((rv.height * 0.85) / pageSize.height, { x: 0, y: 0 });
			var dv = draw.viewbox();
			draw.move((rv.width - dv.width * dv.zoom ** 2) / 2, (rv.height - dv.height * dv.zoom ** 2) / 2);
		};
		resizeFx();
		...
		$.contextMenu({
			selector: '.page',
			className: 'contextmenu-calendar',
	...
	CalGen.prototype.reset = function() {
		...
		var className = $.getClassName(this.config.cover);
		if (className != 'Cover')
			this.config.cover = new Cover(className == 'Object' ? this.config.cover.config : {});
		
		var className = $.getClassName(this.config.rear);
		if (className != 'Rear')
			this.config.rear = new Rear(className == 'Object' ? this.config.rear.config : {});
		...
		for (var i = 0, y = this.config.start.year; y <= this.config.end.year; y++) {
			...
			for (var m = month.start; m <= month.end; m++, i++) {
				var className = $.getClassName(this.config.pages[i]);
				if (className == 'Object' && $.getClassName(this.config.pages[i].config) == 'Object') {
	...
	CalGen.prototype.save = function() {
		try {
			localStorage.setItem('save', this.saveAs('cgp', true));
	...
	CalGen.prototype.print = function() {
		...
		function setPrint () {
			...
			$(this.contentWindow).on('beforeunload afterprint', closePrint);
			this.contentWindow.focus();
			this.contentWindow.print();
		}

		$iframe.on('load', setPrint).css({
			visibility: 'hidden',
			position: 'fixed',
			right: '0',
			bottom: '0'
		...
		var html = '<body><style>@page{ size: A3; margin: 0mm; } svg{ page-break-after: always; }</style>';
		html += '<title>' + this.toString() + '</title>';
		var iframe = $iframe[0];
		var step = 100 / 15 + 0.1;
		$.createProgressModal('Preparing to print...');

		this.renderAll(function(page) {
			html += page.svg();
			$.updateProgressModal(step);
		}, function() {
			$.updateProgressModal(step);
			html += '</body>';
	...
	CalGen.prototype.saveAs = function(format, returnOutput) {
		var saved = true;
		switch(format) {
			case 'cgp':
				var out = "", arr = LZUTF8.compress(JSON.stringify(this.config));
				for(var i = 0; i < arr.length; i++)
					out += String.fromCharCode(arr[i] ^ 0x89);

				if (returnOutput || false)
					return out;
		
				var file = new File([ out ], this.toString() + '.cgp', { type: 'text/plain; charset=us-ascii' });
				saveAs(file);
				break;

			case 'zip':
				var counter = 0, step = 4;
				$.createProgressModal('Saving as ZIP...');
				...
				var renderFx = function(page) {
					...
					if (counter < globalConfig.pages.length)
						$.updateProgressModal(step);
					zip.file(this.toString() + '.svg', page.toDataURL().split(',')[1], { base64: true });
					if (counter < globalConfig.pages.length)
						$.updateProgressModal(step);
				};
	...
	CalGen.prototype.loadAs = function(format) {
		if (format == 'cgp') {
			new FileChooser(function(file) { file.load(function() { self.__load__(this.result) }, 'text') }, { format: '.cgp' }).open();
	...
	CalGen.prototype.__load__ = function(data) {
		if (data == null || (data != null && data.version == this.config.version))
			return;

		var arr = data.split('');
		var out = [];
		for(var i = 0; i < arr.length; i++)
			out[i] = arr[i].charCodeAt() ^ 0x89;

		try {
			out = JSON.parse(LZUTF8.decompress(Uint8Array.from(out)));
		} catch(err) {
			console.error(err);
			return;
		}

		this.config = globalConfig = $.extend(true, { }, out);
		this.reset();
		this.render();
	};
	...
	CalGen.prototype.forward = function(renderFx) {
		if (!$('#prev_page').data('enabled')) {
			$('#prev_page').data('enabled', true).tooltip();
			$('#prev_page > i').fadeTo(0, 1);
		}

		if (pageIndex < this.config.pages.length) {
			pageIndex++;
			if (pageIndex == this.config.pages.length) {
				$('#next_page > i').fadeTo(0, 0);
				$('#next_page').tooltip('disable').data('enabled', false);
			}
	...
	CalGen.prototype.backward = function(renderFx) {
		if (!$('#next_page').data('enabled')) {
			$('#next_page').data('enabled', true).tooltip();
			$('#next_page > i').fadeTo(0, 1);
		}

		if (pageIndex > -1) {
			pageIndex--;
			if (pageIndex == -1) {
				$('#prev_page > i').fadeTo(0, 0);
				$('#prev_page').tooltip('disable').data('enabled', false);
			}
	...
	CalGen.prototype.render = function(renderFx) {
		...
		if ($('#page_number').text() != str) {
			$('#page_number').text(str);
		}
	...
	var Cover = (function() {
		...
		function loadResources(renderFx, callback) {
			var that = this;
			var fx = function() { that.render(renderFx, callback); };
			var resources = [ this.config.image, this.config.font ];

			for (var i = 0; i < resources.length; i++) {
				if (!resources[i].loaded) {
					resources[i].load($.getClassName(resources[i]) != 'Font' ? fx : { success: fx, append: this.$style });
					return false;
				} else if ($.getClassName(resources[i]) == 'Font' && resources[i].src != 'built-in') {
					$('#' + resources[i].id, this.$style).addClass('keep');
				}
			}

			$(':not(.keep)', this.$style).remove();
			$('.keep', this.$style).removeAttr('class');
		...
		Cover.prototype.render = function(renderFx, callback) {
			...
			draw.svg(this.$style.clone().html());
			...
			fontSize = width * this.config.fontSize / measureText(text, fontFamily, fontSize).width;
	...
	var Rear = (function() {
		...
		function loadResources(renderFx, callback) {
			var that = this;
			var fx = function() { that.render(renderFx, callback) };
			var resources = [ this.config.font ];
			...
			$(':not(.keep)', this.$style).remove();
			$('.keep', this.$style).removeClass('keep');
		...
		Rear.prototype.render = function(renderFx, callback) {
			...
			var text = globalConfig.title;
			var fontFamily = this.config.font.name;
			var fontSize = width * this.config.fontSize / measureText(text, fontFamily, this.config.fontSize).width;
			var measure = measureText(text, fontFamily, fontSize);
			...
			width = 800;
			text = globalConfig.subtitle;
			fontSize = width * this.config.fontSize / measureText(text, fontFamily, this.config.fontSize).width;
			measure = measureText(text, fontFamily, fontSize);
			draw.plain(text || '')
			...
			var step = 0;
			if ($.hasProgressModal()) {
				step = Math.floor((100 - $.getModalProgress()) / 12);
			}

			var y = 0, x = 0;
			var pageFx = function() {
				let posX = (342 + x * 332) / 0.16;
				let posY = (900 + y * 464) / 0.16;
				globalConfig.pages[y * 4 + x].render(function(page) {
					draw.nested().svg(page.svg())
						.scale(0.16)
						.attr({ x: posX, y: posY });
				}, function() {
					x++;
	...
	var Page = (function() {
		...
		function loadResources(renderFx, callback) {
			var that = this;
			var success = function() { that.render(renderFx, callback) };
			var failure = function() {
				$.snackbar('Can\'t load resource ' + resources[i].src, 'danger');
				that.render(renderFx, callback, true);
			};
			var loaderOpt = { success: success, failure: failure, append: this.$style };
			var resources = [
			...
			if (this.config.artwork.images != null) {
				resources = $.merge(resources, this.config.artwork.images);
			}

			for (var i = 0; i < resources.length; i++) {
				if (!resources[i].loaded) {
					resources[i].load($.getClassName(resources[i]) != 'Font' ? success : loaderOpt);
					return false;
				} else if ($.getClassName(resources[i]) == 'Font' && resources[i].src != 'built-in') {
					$('#' + resources[i].id, this.$style).addClass('keep');
				}
		...
		Page.prototype.render = function(renderFx, callback, force) {
			...
			var draw = self.getBlankPage();
			draw.defs().svg(this.$style.html());

			var year = this.config.year;
			var month = this.config.month;
			var fontSize = this.config.ribbon.fontSize;
			var fontFamily = this.config.ribbon.font.name;
			...
			draw.image(this.config.ribbon == null ? '' : this.config.ribbon.background.src)
			...
			if (this.config.ribbon != null && $.getClassName(this.config.ribbon.text) == 'String') {
				var text = this.config.ribbon.text;
				var measure = measureText(text, fontFamily, this.config.ribbon.fontSize);
				fontSize = Math.round(this.config.ribbon.fontSize * (754 / measure.width));
				measure = measureText(text, fontFamily, fontSize);
				fontSize = (measure.height > 175 ? Math.round(175 * fontSize / measure.height) : fontSize);
				measure = measureText(text, fontFamily, fontSize);
				draw.plain(text)
					.addClass('page-ribbon-text')
					.style({ font: fontSize + 'px "' + fontFamily + '"' })
					.attr({
						x: 1107 + (814 - measure.width) / 2 - measure.margin.x,
						y: 1680 + (257 + measure.height) / 2 - measure.margin.y / 4,
			...
			var artStyle = this.config.artwork.style;
			var artVariant = this.config.artwork.variant;
			var tile = tiles[artStyle];
			var i = 0;

			do {
				var border = this.config.artwork.border;
				var size = Array.isArray(tile.size) ?
					{ width: tile.size[0], height: tile.size[1] } :
					{ width: tile.size,    height: tile.size    };

				if (border.width > 0) {
					draw.rect(size.width + 2 * border.width, size.height + 2 * border.width)
						.fill(border.color != '' ? border.color : '#000')
						.attr({
							x: 1400 + tile.shift[artVariant][i][0] - border.width,
							y: 1990 + tile.shift[artVariant][i][1] - border.width
						});
				}
				...
				draw.image(this.config.artwork.images[i].src)
					...
					.orElse('dummy')
					.attr({
						x: 1400 + tile.shift[artVariant][i][0],
						y: 1990 + tile.shift[artVariant][i][1]
					});
			} while(++i < artStyle);
		...
		function getLastSunday(y, m) {
			return createDate(y, m, getMonthLength(y, m) - getDayOfWeek(y, m, getMonthLength(y, m)) - 1);
		}
		...
		function isHoliday(y, m, d) {
			if (typeof holiday === 'undefined' || (typeof holiday !== 'undefined' && holiday.year != y)) {
				...
				for (var i = 0; i < locale[globalConfig.lang].timeChange.length; i++) {
					var timeChangeDate = getLastSunday(y, locale[globalConfig.lang].timeChange[i].month);
	...
	$.showOverlay = function(isSpinning) {
		isSpinning = isSpinning == null || isSpinning;
		...
		if ($('#overlay').length) {
			...
			var resizeFx = function() { $('#overlay-spinner').position({ of: $('#overlay') }) };
			$('#overlay').data('resizeFx', resizeFx).show();
			$(window).resize(resizeFx);
			resizeFx();
		}
	...
	$.hideOverlay = function(destroy) {
		if (destroy || false) {
			$(window).off('resize', $('#overlay').data('resizeFx'));
			$('#overlay').remove();
		} else {
			$('#overlay').hide();
		}
	...
	$.createProgressModal = function(title) {
	...
	$.updateProgressModal = function(val) {
		if (!$('.modal-body').length)
			return;

		val = val || 1;
		var $prograssBar = $('.modal-body .progress-bar');
		var newValue = parseFloat($prograssBar.attr('aria-valuenow')) + parseFloat(val);
		$prograssBar
	...
	$.hasProgressModal = function() {
		return !!$('.modal-body').length;
	}
	
	$.getModalProgress = function() {
		return parseFloat($('.modal-body .progress-bar').attr('aria-valuenow'));
	}

	$.removeProgressModal = function() {
		$(window).off('resize', $('.modal-dialog').data('posFx'));
	...
	$.snackbar = function(text, options) {
		...
		$snackbar.position({
			...
			of: window
		}).toggleClass('d-inline d-none').fadeIn(options.fade, function() {
...
var ImageEditor = (function() {
	...
	*/
	...
	function ImageEditor(img, parent) {
...
var Font = (function() {
	...
	Font.prototype.check = function() {
		...
		var $style = $('#' + this.id, this.options.append);
		if (this.options.append != document.head && $style.length > 0)
			$(document.head).append($style.clone());
	...
	Font.prototype.load = function(options) {
		...
		if (this.src == 'built-in') {
			loadEnd.call(this);
		} else if ($.getClassName(this.src) == 'File') {
			loadFile.call(this);
		} else if (Font.isSupportedExtension(this.src) || this.src.indexOf(';base64,') > 0) {
			loadURL.call(this);
	...
	Font.isSupportedExtension = function(src) {
		return formats.some(function(format) {
			return src.length - src.lastIndexOf(format.ext) == format.ext.length;
		});
	};
	...
	function loadLink() {
		...
		var $link = $('<link>', {
			...
			type: 'text/css',
			rel: 'stylesheet'
		}).on('load readystatechange', function() { loadEnd.call(that) });
		$(this.options.append).append($link);
		$(document.head).append($link.clone());
	}
...
function measureText(text, fontFamily, fontSize, fontUnit) {
	if (text == null || (text != null && text.length == 0) || fontFamily == null || fontSize == null)
